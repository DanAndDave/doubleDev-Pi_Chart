...
| 9 | `audit-docs-debt` — **shipped**, archived `2026-09-25` | The documentation debt below, reconciled in both directions: every `CM_*` the README claims against every one `src/` reads, the port scoped to what the code consults, the model row rewritten for today's provenance, the bundle write admitted, and the Thread Store's real cost stated three times over. `skip_specs`. | — |
