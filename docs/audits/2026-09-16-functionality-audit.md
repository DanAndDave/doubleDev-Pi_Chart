...
- **[INFERENCE] Prompt-cache defeat.** Volatile per-Call retrieval is placed ahead of the stable tail (`src/assembler.ts:118-167` before `:169`). Prefix caching means any change to the recalled, curated or structure parts invalidates the cache for the whole message body, and recall changes every Call by construction. Sessions on this machine otherwise run 91–93% `cacheRead`. Moving the volatile parts to sit just before the current Turn would make the body miss only when the tail slides. Worth measuring before committing: ADR-0001 already names prompt-cache cost as a reason a stateless-per-turn design was rejected.
...
| 7 | `single-injector-enforcement` | The one specified `SHALL` with no code behind it. Resolved during planning: every memory backend injects into the **system prompt**, not the message array (`omp://memory.md`, `omp://mnemosyne-memory-backend.md`), so the content lands in the Floor and the Assembler cannot filter it. The requirement is rewritten over the Context Window, enforced by detection, an unmissable report, and a per-Call record. | — |
| 8 | `pack-order-cache` | The `[INFERENCE]` prompt-cache finding, shaped as measurement before any reordering, with the decision rule fixed in advance. | — |
