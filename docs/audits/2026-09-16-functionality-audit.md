...
| 12 | `assembler-shape` | Structural findings from ticket 1's own code review, behind byte-identical Packs: one `Budget` type, one selection rule serving both passes, Elision in its own module. `skip_specs`, `needs-triage`. | — |
