...
| Worst eight consecutive Turns — the default `PICHART_TAIL_TURNS` | **974,861** |
...
`readJournal` groups each Journal into Turns, each Turn is sized with the project's own `approximateTokens` (`src/assembler.ts:290-296`), and an eight-Turn window — the default `PICHART_TAIL_TURNS` — slides across every Journal. The maximum lands on one session, `2026-09-16T04-20-35…01a0a871`, the session that wrote this project, at Turns 7–14:
...
**`PICHART_PG_PORT` is documented and read by nothing in `src/`.** `README.md:131` offers it as an override; `compose.yaml:10` honours it; `DEFAULT_DATABASE_URL` hardcodes `localhost:55432` (`src/config.ts:51-52`) and `loadConfig` never consults it (`:54-70`). `PICHART_PG_PORT=5555` plus `/pi-chart setup` starts Postgres on 5555 while the extension dials 55432, so nothing is ingested, nothing recalled, no curated knowledge and no durable Accounting — and `/pi-chart` reports `thread store  not reachable … run pi-chart setup`, naming the command just run. Build the default URL from the variable or drop the claim.
...
| 3 | `store-hygiene` — **shipped**, archived `2026-09-21` | Codebase set once, incremental batched ingest, `turns.ingested_at` and retention, per-Store deadlines, `runProcess` timeout, the Journal miss, `PICHART_PG_PORT` in code, the Concept candidate-set tiebreak, and the connection pool that is never closed. | — |
...
| 9 | `audit-docs-debt` — **shipped**, archived `2026-09-25` | The documentation debt below, reconciled in both directions: every `PICHART_*` the README claims against every one `src/` reads, the port scoped to what the code consults, the model row rewritten for today's provenance, the bundle write admitted, and the Thread Store's real cost stated three times over. `skip_specs`. | — |
| 10 | `concept-vector-provenance` — **shipped**, archived `2026-09-25` | The unowned finding below: a Concept vector now records the model that made it, is ranked only against that model's, and is re-embedded by the pass that indexes a new one. What a search could not see is counted and carried on the Call. | — |
| 11 | `pack-prefix-stability` — **shipped**, archived `2026-09-25` | The remedy ticket 8 opened. A Pack leads with the longest run of messages the harness itself sent, then the assembled parts, then the rest of the tail, then the prompt: median cached Pack **0.0% → 91.0%**, a governed Call carrying curated knowledge **13,934 → 6,298** tokens charged. ADR-0006. | — |
| 12 | `assembler-shape` — **shipped**, archived `2026-09-25` | Structural findings from ticket 1's own code review, behind Packs checked byte for byte: one `Budget` type, one selection rule serving both passes, Elision in its own module. `skip_specs`. | — |

Every ticket in this table is shipped and archived.

**Found after it, and owned by nothing yet.** Against a Docker-backed Thread Store, `session_shutdown` exceeds the harness's 2,000 ms handler budget and the harness reports `handler timed out after 2000ms` on every session — the shutdown awaits the ingest and the embedding pass it started, and a Postgres in a container is slow enough to miss the budget where the scratch one on a socket is not. Reproduced with the code as it stood before `rename-to-pi-chart` against the same store, so it is older than that change. What it costs is unmeasured: the ingest is awaited precisely so a headless run does not exit mid-write (`structure-freshness`), and a timed-out handler may be a Turn whose Journal reached the Store and whose embedding did not.
...
**What ticket 11 measured, once shipped.** The remedy, with the same instrument. A Pack now opens with the longest run of messages the harness itself supplied, and over seven governed Conversations collected under that order the median cached Pack share is **91.0%** against the baseline's 0.0%. A governed Call carrying curated knowledge costs **6,298** tokens at the provider's multipliers against **13,934** — 2.2× less, and about what an ungoverned Call costs for a Pack a third the size — while uncached input falls from **36.1%** of everything charged to **5.7%**. What decides a Call is whether it has a prefix at all: none, 0.0% over 19 Calls; under 1,024 tokens, 90.0% over 11; above it, 90.0% over 21. Two things the plan did not have: the harness marks its own messages with a symbol property, so a faithful Thread Store copy compares unequal and the run has to be handed back as the harness's own objects; and the run must end at a Turn boundary, because a call separated from its result is a window the provider refuses. ADR-0006.

**What ticket 12 measured, once shipped.** A restructuring is only safe if it can be checked, so it was: every Journal on this machine assembled at four configurations — **2,656 Packs**, 463 of them reduced by the ceiling and 238 carrying something elided — serialised before and after each step. The `Budget` type and the single selection rule are byte-identical across all of them; moving Elision into its own module is byte-identical too. Reconciling the two elision families is not, and was never going to be: the marker's wording changes, and **287** Packs differ by it and by the head and tail either side, **none** of them in what it selected.

Two defects fell out of looking that closely. A message shortened for its metadata alone carried its text twice — the head and the tail covering the whole of it — on **141** Packs, the worst carrying 10,260 characters where 5,165 was the content; and **315** markers reported a negative number of tokens elided, which is the same arithmetic seen from the other side. Both are fixed. The Assembler is 1,698 lines to 1,459, and what left it is Elision and the token estimate.

Documentation debt, owned by `audit-docs-debt`:

- `PICHART_PG_PORT` is documented and unread (`README.md:131`). The code fix is `store-hygiene`'s; the wording must agree with whichever way that lands.
- `PICHART_EMBED_MODEL` is read and undocumented (`src/embedder-worker.ts:17`); a wrong width makes every batch throw into background stderr, which is the failure mode `src/bun-runtime.ts:9-14` set out to eliminate for `PICHART_BUN`.
...
Found while closing that debt, and since owned and shipped:

- **Concept vectors carry no model provenance.** `recall-fidelity` gave Turns an `embedding_model` per vector, so a same-width model swap excludes the strangers from ranking and re-embeds them. `concept_sections` had no such column: `searchConcepts` ordered by distance with no model predicate, and `embedSections` re-embedded a section only when its text hash moved. So after a same-width swap curated knowledge ranked one model's query against another model's vectors until the bundle changed or the index was dropped — the failure mode `recall-fidelity` removed for Turns, still present for Concepts. Owned by `concept-vector-provenance`.

**What that measured, once shipped.** Two 384-dimension models over the vendored bundle, so no width check could refuse the swap (`scripts/measure-model-swap.ts`). Asking "how is gross margin calculated for a period" of an index holding the other model's vectors, the old behaviour carried **nothing** and reported **8 refusals** at 0.820–0.916, against **5 hits from 0.197** when model and index agree: the Concept that answers the question presented as a distant near-miss, so the Call's record blamed the relevance threshold, and the indexing pass repaired **0 of 33 sections** because no text had changed. With provenance recorded, the same search carries nothing and says **8 concepts unsearched**, the next pass re-embeds **33 sections in 3.0s**, and swapping back returns the identical five Concepts at the identical distances.
