...
This slice gives recall a threshold: a Turn must be similar enough to be worth carrying, not merely more similar than the alternatives.
...
- `thread-store`: requirement *Retrieval ranks turns by similarity to the prompt* gains a relevance threshold, so retrieval returns what is relevant rather than what is nearest.
...
- **Unblocks judgement on `cross-conversation-recall`:** widening retrieval across Conversations before it had a relevance threshold would have multiplied the noise this slice removes.
