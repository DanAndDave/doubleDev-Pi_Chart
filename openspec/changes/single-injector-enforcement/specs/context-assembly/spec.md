...
Because the harness injects recalled content into the Floor rather than into the message array, the Assembler cannot remove it: the Floor is by definition the part of a Context Window the Assembler does not supply. The invariant is therefore enforced by detection and disclosure rather than by filtering. The system SHALL check the backend's state when a Conversation starts, SHALL report unmissably when the backend is active, and SHALL treat a backend that cannot be interrogated as unconfirmed rather than as off. The state SHALL be recorded for each Call, so that a Conversation run with two injectors is diagnosable afterwards rather than only suspected.
